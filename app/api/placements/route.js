...
function applyRuntimeCorrections(sourceCompanies) {
  ...
  upsert({ slug: "qualcomm-hw-2027", name: "Qualcomm", role: "Full-Time HW — 2027 Campus Drive", industry: "Semiconductors / Hardware Engineering / Embedded Systems", applicationStatus: "Need Info", appliedDate: "2026-08-26", currentStage: "Official RVCE update on 27-Aug-2026 changes the 3:00 PM Qualcomm OA to remote. Qualcomm will shortlist eligible students and only shortlisted candidates receive the test link. The attached Qualcomm list includes Puneet / 1RF23CS119 but the Resume Link field is blank; a same-day RVITM placement note says only entries with resumes in the field are eligible. No separate Qualcomm test-link email was found as of the 1 PM recheck.", nextAction: "Check Gmail for a Qualcomm test link before 3:00 PM. If a link arrives, take the remote OA from a convenient location using a webcam-enabled laptop. If no link arrives, do not assume eligibility; the official attached list currently has no resume for Puneet and RVITM says resume-populated entries are eligible.", deadline: null, timeline: [{ stage: "HirePro registration deadline passed", date: "27 August 2026, 8:00 AM IST; completion unverified" }, { stage: "Remote Online Assessment", date: "27 August 2026, 3:00 PM; only shortlisted candidates receive the test link" }], source: "Official RVCE email + attached Qualcomm list dated 27-Aug-2026, corroborated by RVITM placement WhatsApp", notes: "Puneet is present in the attached Qualcomm list, but the Resume Link column is blank. RVITM explicitly posted that only students who put resumes in the field are eligible. Use receipt of the company test link as the clearest evidence of personal OA eligibility." });
...
function buildAnnouncements(companies) {
  ...
  const announcements = [];
  if (bySlug.get("qualcomm-hw-2027")?.applicationStatus === "Need Info") announcements.push({ id: "qualcomm-oa-2026-08-27", title: "Qualcomm OA today at 3:00 PM — remote, test link only if shortlisted", message: "Official RVCE update makes today's 3:00 PM Qualcomm OA remote. Only shortlisted candidates receive the test link. Puneet is in the attached Qualcomm list, but the Resume Link field is blank and RVITM says only resume-populated entries are eligible. Check Gmail for the Qualcomm test link before 3:00 PM and use a webcam-enabled laptop if it arrives.", date: "2026-08-27", type: "warning" });
...
export async function GET() {
  ...
  return Response.json({
    ...
    meta: {
      ...
      notice: "Placement records combine the primary tracker with authoritative overrides and newer official updates. Qualcomm's 27-Aug 3:00 PM OA is now remote; only shortlisted students receive the test link. Puneet is in the attached Qualcomm list but its Resume Link field is blank, and RVITM says only resume-populated entries are eligible, so personal OA access remains unconfirmed until a test link arrives or placement staff confirms eligibility. Walmart is Applied but postponed to Friday 28-Aug, time/venue TBD. Juspay is Applied with OA Friday 28-Aug, time/venue TBD. Coupa closed 26-Aug without a completed Puneet registration. Cargill is Applied with the 31-Aug 2:00 PM test next if shortlisted. Qnance is Applied. TekoraAI is a new CSE-only internship opportunity with 4 free unpaid seats, nomination/task details TBD, and no FTE hiring per the direct company email. IDFC is Not Selected; InMobi is Not Applicable; Sama is Not Applied; ShareChat/Eurofins are Not Shortlisted; Sartorius is Not Selected; AMD/Dover/Kinaxis/Coupa are Not Applied. Google and Flipkart remain excluded unless a fresh official notice appears.",
    },
